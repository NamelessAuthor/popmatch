...
d = c[~c['method'].isin(['Bipartify LR', 'GLM'])]
plt.scatter(d['diff'].abs(), d['smd'])

d = c[c['method'].isin(['Bipartify LR', 'GLM'])]
plt.scatter(d['diff'].abs(), d['smd'], c='C1')
plt.xlabel('A2A')
plt.ylabel('SMD')
txts = []
for i, txt in enumerate(c['target']):
    t = plt.gca().annotate(c.iloc[i]['method'], (np.abs(c.iloc[i]['diff']), c.iloc[i]['smd']), fontsize=10)
    txts.append(t)
adjust_text(txts, force_text=(0.2, 0.4), force_static=(0.2, 0.4), force_explode=(0.2, 0.7))
plt.tight_layout()
plt.axhline(y=0.1, color='r', linestyle='--')
plt.savefig('diff_vs_smd_vs_method.pdf')
plt.close()
